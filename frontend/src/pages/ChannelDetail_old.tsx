...
function VideoPlayer({ state, hlsUrl }: any) {
  const isLive = state === 'LIVE_ONLY' || state === 'TV_LIVE_RUNNING'
  const isVod  = state === 'TV_VOD_RUNNING'

  if (!isLive && !isVod) {
    return (
      <div style={{ aspectRatio:'16/9', background:'#000', borderRadius:'var(--r-lg)', border:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:10 }}>
        <div style={{ fontSize:32, opacity:0.2 }}>◉</div>
        <span style={{ fontFamily:'var(--mono)', fontSize:11, color:'var(--text-3)', letterSpacing:'0.1em' }}>NO SIGNAL</span>
      </div>
    )
  }
      ...
      <div style={{ padding:'32px 36px', animation:'fadein 0.25s ease' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24, flexWrap:'wrap', gap:12 }}>
          ...
          {isTV && (
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              ...
              <Btn variant="danger" size="sm" icon="●" onClick={() => action('take-live','Take Live requested')}>Take Live</Btn>
              <Btn variant="blue" size="sm" icon="⏎" onClick={() => action('return-to-vod','Returning to VOD')}>Return to VOD</Btn>
            </div>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:1, marginBottom:24, background:'var(--border)', borderRadius:'var(--r-lg)', padding:3, width:'fit-content' }}>
          {(['preview','ingest','settings'] as const).map(t => (
            <button key={t} onClick={() => setTab(t as any)} style={{ padding:'6px 18px', borderRadius:'var(--r)', border:'none', cursor:'pointer', fontSize:13, fontWeight:tab===t?600:400, textTransform:'capitalize', background:tab===t?'var(--bg-raised)':'transparent', color:tab===t?'var(--text)':'var(--text-2)', transition:'all 0.12s', fontFamily:'var(--sans)' }}>{t}</button>
          ))}
        </div>

        {tab === 'preview' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 260px', gap:16, alignItems:'start' }}>
            <VideoPlayer state={currentState} hlsUrl={playback?.hls_url}/>
          ...
          <div style={{ maxWidth:520 }}>
            <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', padding:22 }}>
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <select
                  defaultValue={channel.timezone || 'UTC'}
                  id="tz-select"
                ...
                >
                  {[
                    ...
                    'America/Jamaica','America/Barbados','America/Trinidad','America/Guyana',
                    'Europe/London','Europe/Paris','Europe/Berlin','Europe/Rome','Europe/Madrid',
                    'Asia/Dubai','Asia/Kolkata','Asia/Singapore','Asia/Tokyo','Asia/Shanghai',
                    'Pacific/Auckland','Pacific/Sydney','Africa/Lagos','Africa/Nairobi',
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <select
                  defaultValue={channel.return_strategy || 'as_clock'}
                  id="rs-select"
                  style={{ width:'100%', background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--r)', color:'var(--text)', padding:'9px 12px', fontSize:13 }}
                >
                  ...
                  <option value="filler_then_resume">Filler then Resume</option>
                </select>
              </div>
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <input
                  id="lt-input"
                  type="number"
                  defaultValue={channel.live_timeout_seconds || 10}
                  style={{ width:'100%', background:'var(--bg)', border:'1px solid var(--border)', borderRadius:'var(--r)', color:'var(--text)', padding:'9px 12px', fontSize:13, boxSizing:'border-box' }}
                />
            ...
            <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:'var(--r-lg)', padding:22, marginTop:16 }}>
              <div style={{ fontSize:14, fontWeight:700, marginBottom:6 }}>Filler Content</div>
              <div style={{ fontSize:12, color:'var(--text-3)', marginBottom:16 }}>
                Shown on loop when no schedule is active. Supports images (JPG, PNG) or video (MP4, MOV).
              </div>

              {channel.filler_path ? (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:'rgba(34,197,94,0.08)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:'var(--r)', marginBottom:14 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <span style={{ fontSize:16 }}>✅</span>
                    <div>
                      <div style={{ fontSize:12, fontWeight:600, color:'#22c55e' }}>Custom filler active</div>
                      <div style={{ fontSize:10, color:'var(--text-3)', fontFamily:'var(--mono)' }}>{channel.filler_path.split('/').pop()}</div>
                    </div>
                  </div>
                  <button onClick={async () => {
                    try {
                      await api.delete(`/channels/${id}/filler`)
                      setChannel({ ...channel, filler_path: null })
                      showToast('Filler removed — using black screen')
                    } catch { showToast('Failed to remove filler', 'error') }
                  }} style={{ fontSize:11, padding:'5px 10px', borderRadius:6, border:'1px solid rgba(255,77,77,0.3)', background:'rgba(255,77,77,0.1)', color:'#ff4d4d', cursor:'pointer' }}>
                    Remove
                  </button>
                </div>
              ) : (
                <div style={{ padding:'10px 14px', background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:'var(--r)', marginBottom:14, fontSize:12, color:'var(--text-3)' }}>
                  No filler set — using black screen
                </div>
              )}

              <label style={{ display:'block', width:'100%', padding:'10px', borderRadius:'var(--r)', border:'2px dashed var(--border)', textAlign:'center', fontSize:13, color:'var(--text-2)', cursor:'pointer', transition:'border-color 0.15s' }}
              ...
              >
                📁 Click to upload filler image or video
                <input type="file" accept="image/*,video/mp4,video/quicktime,video/x-matroska" style={{ display:'none' }}
                  onChange={async (e) => {
                    ...
                    fd.append('file', file)
                    try {
                      const res = await api.post(`/channels/${id}/filler`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
                      setChannel({ ...channel, filler_path: res.data.filler_path })
                      showToast('Filler uploaded — takes effect on next filler cycle')
                    } catch (err: any) {
