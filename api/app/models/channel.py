...
class Channel(Base):
    ...
    # Priority 2: Filler source settings
    filler_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
    filler_source_type: Mapped[str | None] = mapped_column(String(20), nullable=True)  # playlist, rtmp, hls
    filler_source_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("playlists.id", ondelete="SET NULL"), nullable=True)
    filler_source_url: Mapped[str | None] = mapped_column(Text, nullable=True)  # RTMP/HLS URL
    
    # Priority 3: Contributor limit
    contributor_limit: Mapped[int] = mapped_column(Integer, default=3)
    ...
    filler_playlist = relationship("Playlist", foreign_keys=[filler_source_id])
